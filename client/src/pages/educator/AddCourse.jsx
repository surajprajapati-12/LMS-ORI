    ...
    if(action === 'add'){
      ...
      setChapters(
        chapters.map((chapter)=>
          chapter.chapterId ?{...chapter, collapsed :!chapter.collapsed
          } :chapter
        )
      );
    }
...
<div className='flex md:flex-row flex-col items-center gap-3'>
  ...
  >
    <img
      src={assets.file_upload_icon} // replace with your assets.file_upload_icon if imported
...
<div>
  ...
  <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
    <div className='flex justify-between items-center p-4 border-b'>
      <div className='flex items-center'>
        <img onClick={()=>handleChpater('toggle',chapter.chapterId)}
        src={assets.dropdown_icon} width={14} alt="" className=
        {`mr-2 cursor-pointer trsnsition-all ${chapter.collapsed && '-rotate-90'}`} />
        <span className='font-semibold'>{chapterIndex+1} {chapter.chapterTitle}</span>
      </div>
      <span className='text-gray-500'>{chapter.chapterContent.length}
         Lectures</span>
         <img onClick={()=>handleChpater('remove', chapter.chapterId)}
          ...
          {chapter.chapterContent.map((lecture, lectureIndex)=>(
            <div key={lectureIndex} className='flex justify-between items-center mb-2'>
              <span>{lectureIndex+1} {lecture.lectureTitle} - {lecture.lectureDuration}
                ...
                target='_blank' className='tet-blue-500'> Link</a> - {lecture.isPreviewFree  ? 'Free Preview' : 'Paid'}
              </span>
  ...
  rounded-lg cursor-pointer' onClick={()=>handleChpater('add')}>+ Add 
  Chapter</div>

  {showPopup && (
    <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50' style={{'background':'none'}}>
...
<button className='bg-black text-white w-max py-2.5 px-8 rounded my-4'>ADD
